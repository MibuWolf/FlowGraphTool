...
var ui;
(function (ui) {
    class DialogAlert extends ui.Editor.AlertUI {
        constructor() {
            super();
            this.init();
        }
        setData(data) {
            ...
            if (this._data.content && this._data.content.length > 0) {
                let str = this._data.content.join(",");
        ...
        clickHandler(evt) {
            switch (evt.target) {
                case this.btn_ok:
                    if (this._data.flag) {
                        ui.Alert.alert(core.AlertType.CLOSE_GRAPH_TAB_ITEM, this._data.data);
                    }
                    else {
    ...
    ui.DialogAlert = DialogAlert;
})(ui || (ui = {}));
//# sourceMappingURL=DialogAlert.js.map